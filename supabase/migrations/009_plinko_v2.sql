-- Migration 009: play_plinko v2 — supports row_count and risk_level
--
-- Replaces the original play_plinko(BIGINT, INT) with a new signature
-- play_plinko(BIGINT, INT, INT, TEXT) that accepts row count (8-16)
-- and risk level ('LOW'/'MEDIUM'/'HIGH').
--
-- Multiplier tables are hardcoded from the canonical Plinko payout spec.
-- The RTP-calibrated exponential tilting from v1 is preserved.

DROP FUNCTION IF EXISTS play_plinko(BIGINT, INT);
...
BEGIN
  ...
  -- ── 10. Insert win transaction and credit balance ──────────────────────
  INSERT INTO transactions (user_id, amount, type, game)
